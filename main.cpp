...
string directory_moniter="./peek"; 
ofstream raw_data(directory+"/flowfield.dat");
ofstream step_time(directory+"/stepstoconvergence_timestep.dat");
ofstream input_v_star(directory_ML+"/input_v_star.dat");
ofstream input_u_star(directory_ML+"/input_u_star.dat");
ofstream input_div_u_star(directory_ML+"/input_div_u_star.dat");
...
void output(), output_train(), collocate(), write(double* a, int x, int y, char name[]), write_train(), moniter();
double div_vel();
...
void cal_u_star(){
    ...
    for(int i=1; i<N-1; i++){
        for(int j=1; j<N; j++){
            ...
            // D=....
            D=1.0/Re*(u_e+u_w+u_n+u_s-4*u[i][j])/(4*h*h);
...
void cal_v_star(){
    ...
    for(int i=1; i<N; i++){
        for(int j=1; j<N-1; j++){
            ...
            // D=....
            D=1.0/Re*(v_e+v_w+v_n+v_s-4*v[i][j])/(4*h*h);
...
void output_train(){
    for(int i=1; i<N; i++){
        for(int j=1; j<N; j++){
            //input_u_star<< u_star[i][j]<<"  ";
            //input_v_star<< v_star[i][j]<<"  ";
            input_div_u_star<< (u_star[i][j]-u_star[i-1][j]+v_star[i][j]-v_star[i][j-1])<<"  ";
            output_p<< p[i][j]<<"  ";
        }
    }
    //input_u_star<< endl;
    //input_v_star<< endl;
    input_div_u_star<< endl;
    output_p<<endl;
}
...
void guess_p(){
    ...
    for(int i=0; i<N-1; i++){
        for(int j=0; j<N-1; j++){
            u_st[0][i*(N-1)+j]=u_star[i+1][j+1]-u_star[i][j+1]+v_star[i+1][j+1]-v_star[i+1][j]; 
        }
    }
